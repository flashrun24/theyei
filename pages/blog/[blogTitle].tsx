...
export default function blog() {
  ...
  const mediumUrl =
    'https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@sri_yei'
  const mediumURL1 =
    'https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@sakethraj101'
  const mediumURL2 =
  ...
  useEffect(() => {
    ...
    axios.get(mediumURL5).then((data) => {
      data.data.items.forEach((article: any) => {
        if (StringToSlug(article.title) === blogTitle) {
          setBlog({
            account: data.data.feed.link,
